...
function! s:CreateCache()
  let b:clang_opts = s:GetCLANGOptions()
  call writefile(b:clang_opts, b:cache)
endfunction

function! s:GetCLANGOptions()
  let l:out = split(system('echo | clang -v -E -x ' . b:lang_name . ' -'), "\n")
...
function! s:ReadCache()
  let b:clang_opts = readfile(b:cache)
endfunction

function! s:AppendOptions()
  let b:clang_user_options .= ' ' . join(b:clang_opts, ' ')
endfunction

function! ClearCLANGIncludeCaches()
